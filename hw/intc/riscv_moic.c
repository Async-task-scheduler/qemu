...
uint64_t current(MoicHart* moichart) {
    if (moichart->current.proc_id != 0) {
        return moichart->current.proc_id;
    } else if (moichart->current.os_id != 0) {
        return moichart->current.os_id;
    } else if (moichart->hypervisor_id != 0) {
        return moichart->hypervisor_id;
    }
    return 0;
}
...
int64_t check_other_online(MoicHart* moicharts, uint64_t hart_count, int64_t exclude_idx, uint64_t target_id) {
    ...
    for(i = 0; i < hart_count; i++) {
        ...
        if (current(&moicharts[i]) == target_id) {
            return i;
        }
    }
    return -1;
...
static uint64_t riscv_moic_read(void *opaque, hwaddr addr, unsigned size) {
    ...
    uint64_t target_idx = moic->harts_map[idx];
    uint64_t hart_count = moic->hart_count;
    if (op == FETCH_OP) {
        assert(target_idx < hart_count);
        uint64_t task_id = pq_pop(&moic->moicharts[target_idx].ready_queue);
        if (task_id != 0) {
            moic->moicharts[target_idx].current.task_id = task_id;
            return task_id;
        }
    } else if (op == CURRENT_OP) {
        assert(target_idx < hart_count);
        return current(&moic->moicharts[target_idx]);
    } else if (op == STATUS_OP) {
        // info_report("target_idx: %ld", target_idx);
        assert(target_idx < hart_count);
        uint64_t value = 0;
        value |= moic->moicharts[target_idx].cause;
        value |= (moic->moicharts[target_idx].count << 4);
        return value;
    } else {
...
static void riscv_moic_write(void *opaque, hwaddr addr, uint64_t value, unsigned size) {
    ...
    uint64_t actual_idx = moic->harts_map[idx];
    if (op == ADD_OP) {
        assert(actual_idx < hart_count);
        uint64_t priority = (value >> 1) % MAX_PRIORITY;
        // info_report("ADD: hart: %d, Already online, target_idx: %ld", idx, target_idx);
        pq_push(&moic->moicharts[actual_idx].ready_queue, priority, value);
        ...
        if (value == 0) {
            // os -> non
            uint64_t src_task_id = 0;
            moic->harts_map[idx] = hart_count;
            moic->moicharts[actual_idx].count -= 1;
            qemu_log_mask(LOG_UNIMP, "hart %d exit", idx);
            if (moic->moicharts[actual_idx].count == 0) {
                // Store information into the memory.
                qemu_log_mask(LOG_UNIMP, "idx: %d, all harts exit os", idx);
                src_task_id = moic->moicharts[actual_idx].current.os_id;
                store_ready_queue(src_task_id, &moic->moicharts[actual_idx].ready_queue);
                store_device_cap(src_task_id, moic->moicharts[actual_idx].device_cap);
                store_send_cap_queue(src_task_id, &moic->moicharts[actual_idx].send_cap);
                store_recv_cap_queue(src_task_id, &moic->moicharts[actual_idx].recv_cap);
                moic->moicharts[actual_idx].current.os_id = 0;
            }
        } else if (value != 0) {
            // non -> os
            int64_t online_idx = check_other_online(moic->moicharts, hart_count, idx, value);
            if (online_idx >= 0 && online_idx < hart_count) {
                moic->harts_map[idx] = online_idx;
                moic->moicharts[online_idx].count += 1;
                qemu_log_mask(LOG_UNIMP, "hart: %d, Already online, online_idx: %ld", idx, online_idx);
                return;
            }
            uint64_t choose_idx = hart_count;
            if (moic->moicharts[idx].count == 0) {    // search the local idx
                choose_idx = idx;
            } else {    // search another remote idx
                int i = 0;
                for(i = 0; i < hart_count; i ++) {
                    if (i == idx) {
                        continue;
                    }
                    if (moic->moicharts[i].count == 0) {
                        choose_idx = i;
                    }
                }
            }
            qemu_log_mask(LOG_UNIMP, "hart %d, os is not online, choose idx: %ld", idx, choose_idx);
            moic->harts_map[idx] = choose_idx;
            moic->moicharts[choose_idx].count += 1;
            uint64_t dst_task_id = moic->moicharts[choose_idx].current.os_id = value;
            moic->moicharts[choose_idx].current.proc_id = 0;
            moic->moicharts[choose_idx].current.task_id = 0;
            load_ready_queue(dst_task_id, &moic->moicharts[choose_idx].ready_queue);
            load_device_cap(dst_task_id, moic->moicharts[choose_idx].device_cap);
            load_send_cap_queue(dst_task_id, &moic->moicharts[choose_idx].send_cap);
            load_recv_cap_queue(dst_task_id, &moic->moicharts[choose_idx].recv_cap, &moic->moicharts[choose_idx].ready_queue);
        }
    } else if (op == SWITCH_PROC_OP) {
        assert(actual_idx < hart_count);
        if (value == 0 && moic->moicharts[actual_idx].current.os_id != 0) {
            // process -> os
            uint64_t src_task_id = 0, dst_task_id = 0;
            moic->moicharts[actual_idx].count -= 1;
            int64_t online_idx = check_other_online(moic->moicharts, hart_count, actual_idx, moic->moicharts[actual_idx].current.os_id);
            if (moic->moicharts[actual_idx].count == 0) {
                // Store information into the memory.
                src_task_id = moic->moicharts[actual_idx].current.proc_id;
                store_ready_queue(src_task_id, &moic->moicharts[actual_idx].ready_queue);
                store_device_cap(src_task_id, moic->moicharts[actual_idx].device_cap);
                store_send_cap_queue(src_task_id, &moic->moicharts[actual_idx].send_cap);
                store_recv_cap_queue(src_task_id, &moic->moicharts[actual_idx].recv_cap);
            ...
            if (online_idx >= 0 && online_idx < hart_count) {
                ...
                dst_task_id = moic->moicharts[actual_idx].current.os_id;
                uint64_t choose_idx = hart_count;
                if (moic->moicharts[actual_idx].count == 0) { // the actual_idx can be used directly.
                    choose_idx = actual_idx;
                } else if (moic->moicharts[idx].count == 0) {    // search the local idx
                    choose_idx = idx;
                } else {    // search another remote idx
                    int i = 0;
                    for(i = 0; i < hart_count; i ++) {
                        if (i == idx || i == actual_idx) {
                            continue;
                        }
                        if (moic->moicharts[i].count == 0) {
                            choose_idx = i;
                ...
                moic->harts_map[idx] = choose_idx;
                moic->moicharts[choose_idx].count += 1;
                moic->moicharts[choose_idx].current.os_id = dst_task_id;
                moic->moicharts[choose_idx].current.proc_id = 0;
                moic->moicharts[choose_idx].current.task_id = 0;
            }
        } else if (value != 0 && moic->moicharts[actual_idx].current.os_id != 0 && moic->moicharts[actual_idx].current.proc_id == 0) {
            // os -> process
            qemu_log_mask(LOG_UNIMP, "os -> process");
            uint64_t src_task_id = 0, dst_task_id = 0;
            moic->moicharts[actual_idx].count -= 1;
            int64_t online_idx = check_other_online(moic->moicharts, hart_count, actual_idx, value);
            if (moic->moicharts[actual_idx].count == 0) {
                // Store information into the memory.
                src_task_id = moic->moicharts[actual_idx].current.os_id;
                store_ready_queue(src_task_id, &moic->moicharts[actual_idx].ready_queue);
            ...
            if (src_task_id == 0) {
                qemu_log_mask(LOG_UNIMP, "os are running on more than one hart");
            }
            if (online_idx >= 0 && online_idx < hart_count) {
                moic->harts_map[idx] = online_idx;
                moic->moicharts[online_idx].count += 1;
                ...
                if (moic->moicharts[actual_idx].count == 0) { // the actual_idx can be used directly.
                    ...
                    choose_idx = idx;
                    qemu_log_mask(LOG_UNIMP, "the local idx is empty");
                } else {    // search another remote idx
                    int i = 0;
                    for(i = 0; i < hart_count; i ++) {
                        if (i == idx || i == actual_idx) {
                ...
                assert(choose_idx < hart_count);
                qemu_log_mask(LOG_UNIMP, "choose idx is %ld", choose_idx);
                load_ready_queue(dst_task_id, &moic->moicharts[choose_idx].ready_queue);
                load_device_cap(dst_task_id, moic->moicharts[choose_idx].device_cap);
                load_send_cap_queue(dst_task_id, &moic->moicharts[choose_idx].send_cap);
                load_recv_cap_queue(dst_task_id, &moic->moicharts[choose_idx].recv_cap, &moic->moicharts[choose_idx].ready_queue);
                moic->harts_map[idx] = choose_idx;
                moic->moicharts[choose_idx].count += 1;
                moic->moicharts[choose_idx].current.os_id = moic->moicharts[actual_idx].current.os_id;
                moic->moicharts[choose_idx].current.proc_id = dst_task_id;
                moic->moicharts[choose_idx].current.task_id = 0;
            }
        }
...
static void riscv_moic_realize(DeviceState *dev, Error **errp)
{
    ...
    moic->harts_map = g_new0(uint64_t, hart_count);
    // create output irqs
    ...
    int i = 0;
    for(i = 0; i < hart_count; i++) {
        moic->harts_map[i] = hart_count;
        moic->moicharts[i].count = 0;
        moic->moicharts[i].cause = NORMAL;
